...
const promptAddRole = async () => {
  try {
    // Retrieve existing departments to display for selection
    const departments = await executeQuery('SELECT id, name FROM department');

    // Create choices for the inquirer prompt based on existing departments
    ...
    const roleDetails = await inquirer.prompt([
      ...
      },
      {
        ...
        choices: departmentChoices, // Provide department choices
      },
    ]);
