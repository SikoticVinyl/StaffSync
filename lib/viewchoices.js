...
const calculateTotalBudget = async (departmentId) => {
  try {
    ...
    if (result && result.length > 0 && result[0].total_budget !== null) {
      const totalBudget = result[0].total_budget;
      console.log(`Total budget of Department ID ${departmentId}: $${totalBudget}`);
    } else {
      console.log(`No budget information found for Department ID ${departmentId}`);
    }
  } catch (error) {
    console.error('Error:', error.message);
    throw error;
